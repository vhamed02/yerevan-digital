      ...
      <div className="rounded-xl border border-border bg-surface p-6 text-content-primary sm:p-8 print:rounded-none print:border-black print:bg-white print:p-0 print:text-black">
        <div className="flex items-start justify-between gap-6 border-b border-border pb-6 print:border-black">
          <div>
            ...
            <p className="mt-1 text-xs text-content-muted print:text-black">
              yerevan.digital
              <br />
          ...
          <div className="text-right">
            <p className="font-heading text-2xl font-bold uppercase tracking-wide text-content-primary print:text-black">
              Invoice
            </p>
            <p className="mt-1 font-mono text-sm text-content-muted print:text-black">{invoice.number}</p>
            <div className="mt-2 print:hidden">
              <Badge variant={status.variant}>{status.label}</Badge>
            </div>
            <p className="mt-2 hidden font-semibold print:block">{status.label}</p>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-8 py-6">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-content-muted print:text-black">
              Bill To
            </p>
            <p className="mt-2 font-semibold text-content-primary print:text-black">{storeName}</p>
            {store?.email && <p className="text-sm text-content-secondary print:text-black">{store.email}</p>}
            {store?.phone && <p className="text-sm text-content-secondary print:text-black">{store.phone}</p>}
            {store?.address && <p className="text-sm text-content-secondary print:text-black">{store.address}</p>}
          </div>
          <table className="ml-auto w-fit text-sm text-content-primary print:text-black">
            <tbody>
              <tr>
                <td className="py-0.5 pr-6 align-top text-xs font-semibold uppercase tracking-wider text-content-muted print:text-black">
                  Invoice date
                </td>
        ...
        <table className="w-full border-collapse text-sm">
          <thead className="bg-surface-secondary print:bg-white">
            <tr>
              <th className="border-b border-border px-3 py-2 text-left text-xs font-medium text-content-muted print:border print:border-black print:font-bold print:uppercase print:tracking-wider print:text-black">
                Description
              </th>
              <th className="border-b border-border px-3 py-2 text-left text-xs font-medium text-content-muted print:border print:border-black print:font-bold print:uppercase print:tracking-wider print:text-black">
                Period
              </th>
              <th className="border-b border-border px-3 py-2 text-right text-xs font-medium text-content-muted print:border print:border-black print:font-bold print:uppercase print:tracking-wider print:text-black">
                Amount
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            <tr>
              <td className="px-3 py-3 align-top text-content-primary print:border print:border-black print:text-black">
                Platform commission &mdash; weekly settlement
              </td>
              <td className="px-3 py-3 align-top text-content-primary print:border print:border-black print:text-black">
                {invoice.period_start} &ndash; {invoice.period_end}
              </td>
              <td className="px-3 py-3 text-right align-top print:border print:border-black">
          ...
          <tfoot>
            <tr className="border-t border-border font-semibold print:border-black">
              <td
                className="px-3 py-3 text-right text-content-primary print:border print:border-black print:text-black"
                colSpan={2}
              >
                Total due
              </td>
              <td className="px-3 py-3 text-right print:border print:border-black">
                <CurrencyDisplay
                  amount={Number(invoice.amount)}
                  className="text-base font-bold text-content-primary print:text-black"
                />
              </td>
            </tr>
          </tfoot>
        ...
        <div className="mt-6 rounded-lg border border-border bg-surface-secondary p-4 text-sm text-content-secondary print:rounded-none print:border print:border-black print:bg-white print:text-black">
          {invoice.status === 'paid' && (
            <p>
        ...
        <div className="mt-8 border-t border-border pt-4 text-center text-xs text-content-muted print:border-black print:text-black">
