...
- **`required_fields` format:** the seller config UI (`GatewayConfigModal`) and `configure()`'s validation both read the **DB column**, which must be `[{key, label_hy, label_en}]`. A flat `['account_id', ...]` array is the legacy shape — it renders *zero* inputs, so the gateway silently becomes unconfigurable. Idram carried that shape until 2026-08-04; `ineco` and `converse` still do (both inactive). **Seeder edits do not reach production** — `scripts/deploy.sh` runs `migrate --force` and never seeds, so **any** `payment_gateways` change needs a data migration. This has bitten twice: Idram's broken `required_fields` (`2026_08_04_000001`) and Telcell having **no row at all** from 2026-07-17 to 2026-08-04 (`2026_08_04_000003`), which silently kept a fully-working gateway off every storefront. `AdminPaymentGatewayTest` asserts the migrated, unseeded baseline so a third instance is caught in CI.
